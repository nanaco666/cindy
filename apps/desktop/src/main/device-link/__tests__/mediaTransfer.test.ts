...
vi.mock('node:fs/promises', () => ({ stat: statMock, readFile: readFileMock }));
...
describe('uploadLocalFile — 小文件整体 PUT', () => {
  ...
  it('presign-put 带 size/ext/contentType + relay baseUrl,PUT body 为 ArrayBuffer + Content-Length + acl:private', async () => {
    ...
    expect(apiFetch).toHaveBeenCalledWith(
      PUT_PATH,
      expect.objectContaining({ method: 'POST', body: { size: 100, ext: 'png', contentType: 'image/png' }, baseUrl: 'http://relay.test:3335' }),
    );
    ...
    expect(r).toEqual({ key: KEY, size: 100, contentType: 'image/png' });
  });
  ...
  it('contentType 可显式覆盖', async () => {
    ...
    expect(apiFetch).toHaveBeenCalledWith(
      PUT_PATH,
      expect.objectContaining({ body: expect.objectContaining({ contentType: 'application/x-custom' }) }),
    );
  });
...
describe('uploadLocalFile — 大文件流式 PUT', () => {
  it('超阈值 → body 为 ReadableStream + duplex half,不读进内存', async () => {
    statMock.mockResolvedValue({ isFile: () => true, size: __testing.STREAM_THRESHOLD + 1 });
    // 返回真实 Readable,让真实 Readable.toWeb 产出真实 ReadableStream
    createReadStreamMock.mockReturnValue(Readable.from(Buffer.from('big')));
    okPut();
    await uploadLocalFile('/tmp/big.mp4');
...
describe('uploadBuffer — 内存字节(base64 附件)', () => {
  it('presign-put 带 size/ext/contentType,整体 PUT,返回 key', async () => {
    okPut();
    const r = await uploadBuffer(Buffer.from([1, 2, 3, 4, 5]), { ext: 'png', contentType: 'image/png' });
    expect(apiFetch).toHaveBeenCalledWith(
      PUT_PATH,
    ...
    expect(init.body).toBeInstanceOf(ArrayBuffer);
    expect(init.headers['Content-Length']).toBeUndefined();
    expect(r).toEqual({ key: KEY, size: 5, contentType: 'image/png' });
  });
...
describe('downloadToBuffer', () => {
  it('presign-get + GET 整文件 → Buffer + contentType', async () => {
    ...
    expect(apiFetch).toHaveBeenCalledWith(GET_PATH, expect.objectContaining({ body: { key: KEY } }));
...
describe('removeRemote — best-effort', () => {
  it('成功 → DELETE 带 key', async () => {
    ...
    expect(apiFetch).toHaveBeenCalledWith(DEL_PATH, expect.objectContaining({ method: 'DELETE', body: { key: KEY } }));
  });
