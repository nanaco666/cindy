...
describe('networkSlot · 飞书登录态令牌凭证(source:login-feishu-token)', () => {
  ...
  function makeFeishuSlot(overrides: Partial<NetworkSlotDeps> = {}) {
    return makeSlot({
      getGhost: () => fakeGhost({ network: feishuNetwork }),
      feishuToken: {
        ensure: async () => ({ token: 'uat-fresh' }),
        forceRefresh: async () => ({ token: 'uat-refreshed' }),
      },
      ...overrides,
    });
  }

  it('值取自主机飞书令牌通道并按 format 注入;不查保险库;令牌不回流沙箱', async () => {
    const { slot, fetchImpl, readSecret } = makeFeishuSlot();
    const r = await slot.handleFetchRequest('web-search', { url: FEISHU_URL });
    expect(r.ok).toBe(true);
    const sent = fetchImpl.mock.calls[0][1].headers as Record<string, string>;
    expect(sent.Authorization).toBe('Bearer uat-fresh');
    expect(readSecret).not.toHaveBeenCalled();
    expect(JSON.stringify(r)).not.toContain('uat-fresh');
  });

  it('通道未接线:fail-closed,不发请求', async () => {
    const { slot, fetchImpl } = makeFeishuSlot({ feishuToken: undefined });
    const r = await slot.handleFetchRequest('web-search', { url: FEISHU_URL });
    expect(r.ok).toBe(false);
  ...
  it('未连接飞书 / 授权失效(AUTH_EXPIRED):fail-closed 带重登指引,不发请求', async () => {
    const { slot, fetchImpl } = makeFeishuSlot({
      feishuToken: {
        ensure: async () => ({ error: 'AUTH_EXPIRED' }),
        forceRefresh: async () => ({ error: 'AUTH_EXPIRED' }),
      },
    });
    const r = await slot.handleFetchRequest('web-search', { url: FEISHU_URL });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.message).toContain('重新');
    expect(fetchImpl).not.toHaveBeenCalled();
  });

  it('意识伪造 Authorization 头被剥除,最终值由主机注入', async () => {
    const { slot, fetchImpl } = makeFeishuSlot();
    await slot.handleFetchRequest('web-search', {
      url: FEISHU_URL,
    ...
    const sent = fetchImpl.mock.calls[0][1].headers as Record<string, string>;
    const variants = Object.keys(sent).filter((k) => k.toLowerCase() === 'authorization');
    expect(variants).toEqual(['Authorization']);
    expect(sent.Authorization).toBe('Bearer uat-fresh');
  });

  it('上游 401:forceRefresh 后整链重试一次注入新令牌;第二次仍 401 原样返回', async () => {
    let ensured = 'uat-stale';
    const forceRefresh = vi.fn(async () => {
      ensured = 'uat-refreshed';
      return { token: ensured };
    });
    const fetchImpl = vi.fn(async (_url: string, _init: { headers: Record<string, string> }) =>
      fakeResponse({ status: 401, body: '{"code":99991668}' }));
    const { slot } = makeFeishuSlot({
      fetchImpl: fetchImpl as unknown as NetworkSlotDeps['fetchImpl'],
      feishuToken: { ensure: async () => ({ token: ensured }), forceRefresh },
    });
    const r = await slot.handleFetchRequest('web-search', { url: FEISHU_URL });
    // 两次都 401:结果原样透传给意识(status 401),不无限重试。
    expect(r.ok).toBe(true);
    if (r.ok && 'status' in r) expect(r.status).toBe(401);
    ...
    expect(first.Authorization).toBe('Bearer uat-stale');
    expect(second.Authorization).toBe('Bearer uat-refreshed');
  });
});
