...
export interface AtlassianAccountsMigrationDeps {
  /** 读老 refresh token 明文(safeStorage 解密 jira_refresh_token.enc;不存在 / 解密失败回 null)。 */
  readLegacyRefreshToken(): string | null;
  /** 读老连接信息(jira_connection.json;只消费 email,坏形态回 null)。 */
  readLegacyConnection(): { email?: string | null } | null;
...
export function migrateAtlassianAccounts(deps: AtlassianAccountsMigrationDeps): number {
  ...
  if (vault.read(XD_ATLASSIAN_GHOST_ID, accountsKey) !== null) return 0;

  const refreshToken = readLegacyRefreshToken();
  if (!refreshToken) return 0;

  const connection = readLegacyConnection();
  const email = typeof connection?.email === 'string' && connection.email.length > 0 ? connection.email : null;

  const accountId = randomUUID();
  // rt 先落库再挂清单(与 GhostOauthAccountManager.connectAccount 同顺序,防半身位)。
  if (!vault.store(XD_ATLASSIAN_GHOST_ID, `${XD_ATLASSIAN_SECRET_KEY}-rt-${accountId}`, refreshToken)) {
