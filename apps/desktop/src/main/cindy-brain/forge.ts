...
\`\`\`json
"setup": {
  "requires": [                                     // 1–8 组;组间全部满足(allOf),组内任一满足(anyOf)
...
- **绝大多数意识不需要写本字段**:不声明时主机走启发式——声明过凭证/连接的意识,
  任一项配好即算就绪;什么都没声明的恒就绪。只有启发式判不准(如"必须**同时**配 A
  和 B"、"某个 /kv 参数是硬前置")才需要显式声明。
...
硬规则(主机代码强制,写了也没用的别写):
- **纯 HTML+CSS,零脚本**:\`<script>\`/事件属性/\`<a>\`/iframe/svg 等一律被主机
...
声明 \`settingsHtml\`——主机在侧栏「插件」的详情页里渲染你自绘的设置界面,
...
3. **告知用户去点弹窗**(装入默认沉睡,提醒用户勾"立即开启"或到设置里唤醒);
