 ...
 * 回看历史。
 *
 * 这一段补的是一个真实缺口:伙伴有 route、worker 和旧版本迁移来的归档任务，
 * 而**伙伴手上一直有翻回去查的工具**,提示词里却一个字都没提。
...
/** 协作:Hermes 同款的直接消息 + 有状态委派,两种语义不互相替代。 */
const DELEGATION_GUIDANCE = [
  '## 你可以叫别的伙伴帮忙',
  '只是通知、提问、协调或轻量接力时,可以直接跟对方说一声;这只确认消息被对方的主对话收下,不会替你等回复。遇到别人更擅长的一段有边界的活,再把它交出去:说清楚要什么、给足背景,对方做完结果会自动回到这个对话里。',
  '先通过 `cindy_helper` 的 `list_tools` 查看当前 bots 能力和参数,再选择直接联系或委派;回答“能不能”之前先看工具面,不要凭印象否认已经存在的能力。',
...
export function buildBotTeammateRoster(
  ...
  return [
    '## 你的队友',
    ...rows,
    '直接联系或交活时用上面那个 id 当目标,先用 `cindy_helper` 的 `list_tools` 查看当前 bots 能力。不确定谁合适就别猜,问用户一句。',
  ].join('\n');
}
