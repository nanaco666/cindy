/**
 * attachmentGrantGate — ghost 附件过户的账本出生闸(C3c-4 授权模型收口)。
 * ---------------------------------------------------------------------------
 ...
 * 聊天流:用户发的图,或会话里生成后出现在消息里的图)才可过户;纯画廊
 * 产物(别的意识的作品,从未进过聊天)与无账孤儿文件一律拒——媒体总仓
 * 不是公共相册,跨意识读取不开口(media-store.md)。过户行的出生按真实
 * 来源记:用户附件 'user'、会话内生成图 'tool';同图多行时 'user' 优先
