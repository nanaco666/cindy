...
describe('WorkerThreadTransport', () => {
  ...
  it('normalizes migration.writePage workingDir in the inline worker fallback (#537)', async () => {
    const transport = new WorkerThreadTransport({ useInlineWorker: true });
    try {
      await transport.send('exec', {
        sql: `CREATE TABLE sessions (
      ...
      await transport.send('exec', {
        sql: 'CREATE TABLE migration_meta (key TEXT PRIMARY KEY, value TEXT NOT NULL)',
      });

      await transport.send('tx', {
        name: 'migration.writePage',
        args: {
          sessions: [
            {
              id: 's-win',
      ...
      await expect(transport.send('queryOne', {
        sql: 'SELECT working_dir FROM sessions WHERE id = ?',
        params: ['s-win'],
      })).resolves.toEqual({ working_dir: 'D:/legacy/repo' });
    } finally {
      await transport.close();
    }
  });
