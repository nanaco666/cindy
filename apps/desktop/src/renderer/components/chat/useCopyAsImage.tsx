 ...
 * 因此把光栅化收敛为调用方传入的 `getBlob`(mermaid 走 svgToPngBlob,DOM 块
 * 走 domToPngBlob),hook 统一承担:
 *   - 复制:PNG Blob → ClipboardItem(纯内存,不落盘),成功用图标切换反馈
...
export function useCopyAsImage(getBlob: () => Promise<Blob>): {
