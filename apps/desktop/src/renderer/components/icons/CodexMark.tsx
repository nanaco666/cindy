/**
 * CodexMark —— OpenAI Codex 官方 CLI mark:多瓣圆花轮廓内嵌 `>_` 终端提示符,
 * 蓝紫渐变(#b1a7ff → #7a9dff → #3941ff)固定品牌色(跨主题一致,语义豁免——
 * 同 DESIGN.md Toast 三色逻辑,不走主题 token,不吃 currentColor)。
 *
 * 2026-07-20 由单色 OpenAI 六瓣花结换成彩色品牌 mark(参考 open-design 的
 * agent-icons 处理)。官方资产自带的白色圆角方底被去掉:内联小尺寸场景用透明底,
 * `>_` 镂空直接透出所在背景,明暗主题都成立。渐变 id 用 useId 派生——同屏多个
 * 实例若共用字面量 id,首个实例被卸载/隐藏时其余引用会失效。
 */
...
export function CodexMark({ size = 14, className }: CodexMarkProps) {
