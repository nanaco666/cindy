 ...
 *
 * 所有路径统一为 24px viewBox + currentColor，确保设置页在任意主题下都沿用
 * 现有图标色。品牌路径来自 Simple Icons 16.27.0（CC0-1.0）；xAI 与智谱
 * Mono 路径来自 Lobe Icons 5.14.0（MIT）。
 */
...
type ProviderLogoKind = keyof typeof PROVIDER_LOGO_PATHS | 'anthropic' | 'openai' | 'xd';

export type ProviderLogoRouting = Readonly<
  Record<string, { readonly upstream: string } | undefined>
>;

/** 目录 preset 的地区变体共享同一个供应商品牌，不按展示名做模糊判断。 */
...
/**
 * 从预设创建后 provider id 会改成用户可编辑名称的 slug，所以运行态不能只认 preset id。
 * 上游 host 是随 provider 配置持久化的稳定事实；按域名边界匹配，避免把
 * `openrouter.ai.evil.example` 这类伪后缀误认成品牌。
 */
...
/** 用稳定 id 优先、持久化 upstream 兜底，解析供应商品牌。 */
