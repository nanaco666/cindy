 ...
 *   §14.2 相同(红线 d):打开聚焦 [data-morph-autofocus] → 首个 input → 面板容器,
 *   关闭后焦点归还 trigger。
 *
...
export function MorphPopover({
  ...
  const measure = useCallback(
    (panel: HTMLDivElement, chipRect: DOMRect) => {
      ...
      let targetW: number;
      if (panelWidth) {
        targetW = Math.max(panelWidth, chipRect.width);
      } else {
        panel.style.width = 'max-content';
        targetW = Math.max(panel.offsetWidth, chipRect.width);
      }
      ...
      const targetH = Math.min(panel.offsetHeight, avail);
  ...
  useLayoutEffect(() => {
    ...
    if (open) {
      ...
      // 6) §14.2 焦点:autofocus 标记 → 首个 input → 面板容器
      const focusDelay = reduced ? 0 : MORPH_MS;
      closeTimerRef.current = setTimeout(() => {
        settledRef.current = true;
        const target =
          panel.querySelector<HTMLElement>('[data-morph-autofocus]') ??
  ...
  }, [mounted, open, measure, applyChipGeometry, endBg, endBorderColor, hasCustomGhost]);
  ...
  /** 打开期间的全局关闭手势:outside pointerdown / Esc / 窗口 resize */
  useEffect(() => {
      ...
      {mounted &&
        createPortal(
          <div
            ...
            tabIndex={-1}
            className="group fixed z-50 overflow-hidden border outline-none"
