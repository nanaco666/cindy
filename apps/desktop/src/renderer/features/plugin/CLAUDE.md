...
- Installed data comes from `window.electronAPI.ghosts` and `useInstalledGhosts`. There is no
  builtin/enterprise provisioning metadata anymore: every plugin is user-installed (SkillHub or
  manual `.cindy`), so the page has no origin grouping and no restorable entries.
...
- `__tests__/GhostPluginCard.test.tsx` — installed card action coverage.
...
Run the module tests plus the two adjacent `cindy-brain` adapter tests, then lint changed TypeScript/TSX, run i18n validation, and audit the final diff against the current upstream main ref.
