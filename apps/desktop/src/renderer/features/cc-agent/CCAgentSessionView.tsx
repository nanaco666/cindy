...
export function CCAgentSessionView({
  ...
  const session = serverSession ?? sessionFromList ?? null;
  ...
  useEffect(() => {
    ...
    const unsub = sessionsPush.onPatched(({ sessionId: patchedId, patch }) => {
      if (patchedId !== sessionId) return;
      setServerSession((prev) => (prev ? { ...prev, ...patch } : prev));
  ...
  // model-selector-xhigh-ui-stale fix (2026-04-21): SSoT requires the parent
  // refresh `session` after effort / permissionMode persist, mirroring the
  // model path. ChatInput no longer keeps a local override; it depends on
  // these callbacks to drive UI updates via prop change.
