...
function createNotAllowedCursor(stroke: string): string {
  const encodedStroke = stroke.startsWith('#') ? `%23${stroke.slice(1)}` : encodeURIComponent(stroke);
