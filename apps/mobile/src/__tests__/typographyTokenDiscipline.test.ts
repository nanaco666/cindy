...
describe('typography token discipline', () => {
  it('forbids literal fontSize / lineHeight / fontWeight in styles', () => {
    ...
    for (const rel of files) {
      ...
      src.split('\n').forEach((line, i) => {
        if (/fontSize:\s*\d/.test(line)) violations.push(`${rel}:${i + 1} literal fontSize -> use typeScale`);
        if (/lineHeight:\s*\d/.test(line)) violations.push(`${rel}:${i + 1} literal lineHeight -> use lineHeight token`);
        if (/fontSize=\{\d/.test(line)) violations.push(`${rel}:${i + 1} literal fontSize JSX prop -> use typeScale`);
        if (/lineHeight=\{\d/.test(line)) violations.push(`${rel}:${i + 1} literal lineHeight JSX prop -> use lineHeight token`);
        if (/fontWeight:\s*(['"]|\d)/.test(line)) violations.push(`${rel}:${i + 1} literal fontWeight -> use fontWeight token`);
        if (/letterSpacing:\s*0[,\s}]/.test(line)) violations.push(`${rel}:${i + 1} letterSpacing: 0 is a no-op, remove it`);
      });
    }
  ...
  it('forbids importing Text / TextInput values directly from react-native (use @/components/AppText)', () => {
    ...
    for (const rel of files) {
      ...
      const src = readFileSync(join(ROOT, rel), 'utf8');
      for (const m of src.matchAll(/import\s*\{([^}]*)\}\s*from\s*['"]react-native['"]/gs)) {
        const names = m[1]
          .split(',')
          .map((n) => n.trim())
        ...
        for (const n of names) {
          ...
          if (base === 'Text' || base === 'TextInput') {
            violations.push(`${rel}: imports ${base} from react-native, use @/components/AppText`);
          }
        }
      }
    }
  ...
  it('keeps textStyles presets composed from the typeScale / lineHeight ladders', () => {
    ...
    for (const [name, preset] of Object.entries(textStyles)) {
      expect(sizes.has(preset.fontSize), `textStyles.${name}.fontSize 必须来自 typeScale`).toBe(true);
      expect(heights.has(preset.lineHeight), `textStyles.${name}.lineHeight 必须来自 lineHeight 表`).toBe(true);
      expect(preset.lineHeight, `textStyles.${name} 行高必须大于字号`).toBeGreaterThan(preset.fontSize);
    }
  });

  it('keeps the fontWeight ladder restrained (4 tiers, bold reserved)', () => {
    expect(Object.keys(fontWeight)).toEqual(['regular', 'medium', 'semibold', 'bold']);
  });
});
