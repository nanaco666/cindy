 ...
 * KaTeX 资源只允许由内联 loader 动态注入(双 CDN failover + 超时)。
 */

import { describe, expect, it } from 'vitest';
import {
  MOBILE_KATEX_CDN_SOURCES,
  buildKatexLoaderJs,
...
describe('buildMathWebViewHtml — 零阻塞外链约束', () => {
  ...
  it('公式源码作为首屏内容直接绘制(CDN 挂起也不白屏)', () => {
    expect(html).toContain('<pre>E = mc^2</pre>');
  });

  it('loader 覆盖全部 CDN 源(jsdelivr 主 + npmmirror 备)', () => {
    expect(MOBILE_KATEX_CDN_SOURCES.length).toBeGreaterThanOrEqual(2);
    for (const source of MOBILE_KATEX_CDN_SOURCES) {
      expect(html).toContain(source.css);
      expect(html).toContain(source.js);
    }
  });
...
describe('buildMathWebViewHtml — 主题色净化', () => {
  it('合法 hex / rgba 原样使用', () => {
    const html = buildMathWebViewHtml('x', { background: '#1f1f1e', textPrimary: 'rgba(212, 212, 212, 0.9)' });
...
describe('buildKatexLoaderJs', () => {
  it('包含超时 failover 与 window.katex 就绪检查', () => {
    const js = buildKatexLoaderJs('doRender();');
    expect(js).toContain('attempt(i + 1)');
...
describe('buildSelectableMarkdownHtml — math 文档同守零阻塞约束', () => {
  it('含公式文档:loader 注入、无静态外链、占位源码保留', () => {
    const html = buildSelectableMarkdownHtml('段落 $E=mc^2$ 和\n\n$$\n\\frac{1}{2}\n$$');
    expect(html).not.toMatch(/<link\s/i);
    expect(html).not.toMatch(/<script\s+src=/i);
    expect(html).toContain('data-latex');
    expect(html).toContain('data-katex-display');
    expect(html).toContain(MOBILE_KATEX_CDN_SOURCES[0].js);
  });
