// expo-router 深链拦截。
//
// 飞书 OAuth 回调 lizcn://auth?code=...&state=... 没有对应路由页,默认会落到 expo-router 的
...
//
// 背景(为何 Android 暴露、iOS 不会):iOS 的 ASWebAuthenticationSession 会在会话内捕获
// lizcn://auth 并 inline 完成,router 不会导航到 /auth;Android 上服务器 302 到自定义 scheme
// 常以新 intent 冷启 App,expo-router 就撞上无路由的 /auth。此拦截让两端一致落到 index。
//
// 纯 JS(不进 @expo/fingerprint / 不改 runtimeVersion),可随热更下发。

export function redirectSystemPath({ path }: { path: string; initial: boolean }): string {
  try {
    // path 可能是完整 URL('lizcn://auth?code=...')或路径('/auth?code=...'),统一取出 pathname。
