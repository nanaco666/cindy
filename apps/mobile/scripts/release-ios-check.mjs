...
function nextStepFor(mode) {
  if (mode === 'OTA_OK') {
    return ['→ 只改了 JS/TS,指纹未变:发热更即可', '   pnpm mobile:release:ios:ota -- --execute'];
  }
  if (mode === 'COLD_BUILD_REQUIRED') {
    return ['→ 原生输入变化,指纹已变:必须冷更出整包', '   pnpm mobile:release:ios:local -- --execute'];
  }
  return ['→ 线上无基线(首发或 release.json 未上传):按首次冷更处理', '   pnpm mobile:release:ios:local -- --execute'];
}
