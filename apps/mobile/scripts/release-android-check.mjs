...
// mobile-ota/android/canary-release.json(无 canary 时回退 stable release.json)记录的"上次冷更装机包 runtimeVersion"比对:
//   - 相等         → OTA_OK           (只改了 JS/TS,发热更即可)
//   - 不等         → COLD_BUILD_REQUIRED(动了原生输入,必须冷更出整包)
//   - 读不到基线   → BASELINE_UNKNOWN  (首发 / canary、stable 指针均未上传)
//
// 只读:只算本地指纹 + GET 公开 CDN 上的 canary/stable release 指针,不写、不碰 EAS/NPKG/keystore。
...
function selfhostEnv(region, versionCode) {
  const env = {
    ...process.env,
...
function nextStepFor(mode, region) {
  if (mode === 'OTA_OK') {
    return ['→ 只改了 JS/TS,指纹未变:发热更即可', `   ${formatSelfHostReleaseCommand('android', 'ota', region, { execute: true })}`];
  }
  if (mode === 'COLD_BUILD_REQUIRED') {
    return ['→ 原生输入变化,指纹已变:必须冷更出整包', `   ${formatSelfHostReleaseCommand('android', 'local', region, { execute: true })}`];
  }
  return ['→ 线上无基线(首发或 canary/stable release 指针未上传):按首次冷更处理', `   ${formatSelfHostReleaseCommand('android', 'local', region, { execute: true })}`];
}
...
async function main() {
  ...
  RELEASE_RECORD_CDN = buildReleasePointerLocation({
    cdnBase: CDN_BASE, ossPrefix: '', platform: 'android', channel: 'canary',
  }).url;
  STABLE_RELEASE_RECORD_CDN = buildReleasePointerLocation({
    cdnBase: CDN_BASE, ossPrefix: '', platform: 'android', channel: 'stable',
  }).url;

  const versionCode = readAndroidVersionCode(MOBILE_DIR);
  const env = selfhostEnv(region, versionCode);
  ...
  const baseLabel = baseline.record?.runtimeVersion
    ? `${baseline.record.runtimeVersion}${baseline.record.version ? ` [v${baseline.record.version} versionCode ${baseline.record.buildNumber}]` : ''}`
    : '(无 / 首发)';

  console.log('');
  console.log(`target: mobile 自建线(android, region=${region.authRegion}, ${region.androidPackage})`);
