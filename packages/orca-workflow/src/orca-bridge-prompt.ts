...
export function renderOrcaLeadSystemPrompt(initialWorker?: OrcaInitialWorkerRef | null): string {
  const lines = [
    ...
    '- For a single-role request, after create_worker or send_to_worker returns with the concrete dispatch signals above, your turn is OVER. Produce ZERO output — no summary, no confirmation, no "I\'ve sent...", no "waiting...", nothing.',
    '- If create_worker or send_to_worker fails or lacks those dispatch signals, report the result immediately instead of ending silently. Do not wait or poll for a worker response that was not scheduled.',
