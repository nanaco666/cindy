...
const DESCRIPTION = [
  ...
  '结果包含 request_count / attempted_count / success_count / failure_count / skipped_count、hard limit 快照、确定生成的 user_report，以及每个 label 对应的 worker/session 或失败原因。',
...
function buildUserReport(params: {
  ...
  const base = `本批请求创建 ${params.requestCount} 个 Worker，实际创建成功 ${params.successCount} 个，失败 ${params.failureCount} 个`;
  if (!params.stoppedEarly || !params.limit) return `${base}。请按逐项结果核对每个 Worker 的真实终态。`;
  return `${base}；当前 hard limit 为 ${params.limit.workerHardLimit}，已占用 ${params.limit.occupiedSlots} 个槽位，其中 ${params.skippedCount} 个请求在达到上限后未再尝试。可在协同设置中提高 hard limit、复用已有 Worker，或归档不再需要的 Worker 后分批执行剩余任务。`;
}
  ...
  registry.register({
    ...
    handler: async ({ workers }) => {
      ...
      for (let index = 0; index < workers.length; index += 1) {
        ...
        if (result.errorCode !== 'WORKER_LIMIT_HARD_EXCEEDED') continue;
        ...
        for (const pending of skipped) {
          results.push({
            ...baseResult(pending),
            status: 'skipped',
            error_code: 'WORKER_LIMIT_HARD_EXCEEDED',
            hint: '同批已达到 Worker hard limit，未再调用 host 创建。',
          });
        }
        break;
      }

      const failureCount = workers.length - successCount;
      ...
      return okPayload({
        ...
        ...(stoppedEarly ? { stop_reason: 'WORKER_LIMIT_HARD_EXCEEDED' } : {}),
        ...(limit ? { limit: toWorkerLimitPayload(limit) } : {}),
        user_report: userReport,
        results,
        suggestions: stoppedEarly ? hardLimitSuggestions() : [],
      });
    },
  });
}
