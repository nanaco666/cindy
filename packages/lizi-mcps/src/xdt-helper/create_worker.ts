...
const DESCRIPTION = [
  ...
  '- effort: 可选, reasoning/thinking 强度 (low / medium / high / xhigh / max)。Codex: 映射 OpenAI reasoning effort; Claude Code: 映射 extended thinking token 预算。显式传入时必须匹配所选 model 能力；当前 worker 模型都不把 minimal 作为可选思考档。',
  ...
  "- initial_task: 可选, 创建后立即派给 worker 的第一条消息；dispatch_outcome.wakeKind=queued 表示首条任务已成功入队；dispatch_outcome.kind='session-dispatch' 且 dispatched=false，或 kind='host-send' 且 accepted=false，表示 worker 已创建但首条任务未送达 / 派发失败",
  '',
  '【硬边界】',
