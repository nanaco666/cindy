# Dogfooding 工作流：用 XDMaker 开发 XDMaker
...
XDMaker 本身就是 coding agent 宿主，用它开发它自己（dogfooding）是最紧的产品反馈环。**结论：编码生产环今天端到端可用**——在 worktree 会话里 edit → typecheck → vitest → `git commit` → `gh pr create` 全链路通畅。真正的限制只有一条物理约束（HMR 是 checkout 级的，见下）和两个 P0 缺口（worktree 运行时验证未工具化、新 worktree 无 node_modules，见文末差距清单）。
...
3. **打包版 xdt-maker.app——可选的稳定兜底**。restart 脚本永远不会杀它（其可执行路径不含源码 rootDir）。
...
**agent 永远无法重启宿主**：restart 脚本检测到自己的 ppid 链在 XDMaker dev 进程树内会 exit 1 拒绝（by design）。每次 main / maker-core 迭代预算一次人工触碰。

## 变更类型 → 回路决策表

| 变更类型 | 在哪做 | 验证回路 | 需要人吗 |
|---|---|---|---|
| 小 / 安全的 renderer 改动 | baseRepo 直改会话（不开 worktree） | HMR 即时可见（就在承载会话的 app 里） | 否 |
| 有风险的 renderer / 布局重构 | worktree 会话 | worktree 内 vitest / typecheck + 可选 verify 实例 | 启 verify 实例 |
| main / preload / dbWorker | worktree 会话（首选）或 baseRepo 直改 | 无热更；verify 实例，或 merge 后人工 restart | **是**（restart 永远是人） |
...
| 零依赖 `packages/*` | baseRepo 直改 | HMR | 否 |
...
baseRepo 直改 renderer 的会话是在热更**承载它自己的那个 app**——回路最快，但一次坏改动会把显示会话的窗口改白。此类会话只做小而可 review 的编辑，有风险的 UI 工作升级到 worktree。
...
> 4. 你不能重启桌面端；需要运行时验证时，commit + push 后告诉我，由我启 verify 实例或重启。
...
**按变更类型先验证**（对照上面的决策表），然后落地，两条路线：

**路线 A——PR（首选，产品徽标全程可用）**：agent 在 worktree 会话内

```bash
git add -A && git commit -m "..."
git push -u origin xdt/<name>
gh pr create --draft --base <source-branch> ...
```

然后走正常 GitHub review / merge（仓库有 `review-pr` skill）。`--base` 不能省：省略时 `gh` 会取 `gh-merge-base` 配置、否则退回仓库默认分支（main）——从非 main 源分支创建的 worktree 会把 PR 开到 main，混入源分支的无关分叉（同上一节监控命令的 `<source-branch>` 语义）。

**路线 B——本地 merge 进活 checkout（快速内环）**：人在 baseRepo 终端 `git merge --no-ff xdt/<name>`。renderer 改动经 HMR **立即**生效在实例 A；main / maker-core 改动仍需 `pnpm restart:desktop:remote`（人工）。

## 收尾顺序（顺序错了会吃改动）

1. worktree 内**先 commit（+ push）**。
...
3. merge 后自己删分支：`git branch -d xdt/<name>`——产品从不删分支，会持续积累。
...
| worktree 会话里 skill / 配置陈旧 | `.claude` / `.sivi` 在创建时一次性 copy；之后 baseRepo 的 skill 改动对既有会话不可见 | 开发 skill 本身时用 baseRepo 直改会话，或改完重建 worktree 会话 |
| 「在 worktree 里改了没反应」 | HMR 根在启动 checkout（`vite.renderer.config.ts:222`） | 见心智模型节；verify 实例或 merge-back |
...
| deep link 落错实例 | `xdt-maker://` 后注册者赢（`deepLink.ts`） | 接受现状；知道哪个实例最后注册即可 |
...
- 「改动」tab 非 git 真相，且无 commit / push / PR 按钮——merge-back 全靠终端。
...
**推荐推进顺序**（每步本身就是 dogfooding 任务，按当前能力递增）：Day 0 人工验证闭环（登录 shell 启实例、`gh auth status`、跑一个 trivial worktree 会话到 PR）→ 纯文档契约（本文档 + AGENTS.md 节）→ baseRepo 直改的小 UX 修复练手 → `scripts/dev-worktree.mjs`（P0-1）→ createWorktree 后台 install（P0-2）→ 持久化审批白名单 / auto-stash 确认 UX / `.claude` 重同步（P1）→ git 真相 review tab + commit / PR 按钮（最大件）。
