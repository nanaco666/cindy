...
Orca 是 XDMaker Desktop 内的多 agent 协同能力：一个 **Lead session** 负责拆任务、派活、验收与汇总，多个 **Worker session** 负责并行或串行执行。Worker 是完整会话，不是一次性 subagent；它有自己的模型、effort、工具调用流、上下文与可见历史。
