 ...
 * api / auth / device-link 指向 localhost 三件套,其余字段(oauth broker /
 * heartbeat / slack hook / website / 网关 / 更新链 CDN)照抄 cn 正本(本地
 * 不起这些服务,沿用远程值,消费方各自的"连不上就跳过"分支继续生效)。
...
export function generateEndpointLocalFile({ repoRoot }) {
  ...
  const local = {
    _note:
      ...
      '每次启动整文件重写,手改会丢;localhost 三件套之外的字段照抄 config/endpoint.json。',
    ...source,
