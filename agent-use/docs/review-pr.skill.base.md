...
- **`review` 型 → 并行审查**:
{{#claude}}
  把处理清单里**所有** `review` 型 PR 的审查 subagent **在同一条消息里一次性并行起**:每个用 `Agent` 工具、`subagent_type: "Code Reviewer"`、**加 `isolation: "worktree"`**(每个 PR 一个独立 git worktree,互不干扰)。审查 prompt 按步骤 2「调用模板」拼,其中「代码检出」一行填 worktree 自检出指令(见模板下方说明)——**主工作树全程不 checkout、不切分支**,1.4 在 auto 批处理下由各审查 worktree 里的子 agent 自己完成(`node scripts/review-pr/checkout.mjs <PR> --detach`)。
{{/claude}}
{{#codex}}
...
红线路径命中走 `context.json` 的 `format.redlinePaths` / `format.hitsUpdater`(脚本按 `pr-rules.json` 的 `redlinePaths` 判,路径清单只此一处)。脚本不可用时,对照 `{{AGENT_DOC}}`「设计实现规范」的 xdt-updater 条(规则 21)与 DB migration 条(规则 17)自行识别红线路径,**不要在本 skill 里另存一份路径清单**。
...
{{#claude}}
> **auto 批处理下本步不在主工作树执行**:主 agent 全程不 checkout;各审查 subagent 在自己的 worktree 里跑 `node scripts/review-pr/checkout.mjs <PR> --detach`(detached HEAD、不建分支)完成检出,见「候选批处理」阶段 2。下面的主树 checkout 流程只适用于交互模式。
{{/claude}}
...
{{#claude}}
**本步骤的全部审查工作(下面 12 项框架 0 / 0.5 / 0.6 / 1-9)必须通过 `Agent` 工具起 `Code Reviewer` subagent 完成,主 agent 不直接审。** auto 批处理模式下,多个 `review` 型候选的审查 subagent **在同一条消息里并行起、每个加 `isolation: "worktree"`**(见「候选批处理」阶段 2);交互模式单 PR 则起一个、不加 worktree 隔离(主树已 checkout)。理由:
{{/claude}}
{{#codex}}
...
{{#claude}}
- **专业化**:`Code Reviewer` subagent 针对代码审查任务做过优化,反馈更聚焦、更可执行、不掺无关意见。
{{/claude}}
{{#codex}}
...
{{#claude}}
用 `Agent` 工具,`subagent_type: "Code Reviewer"`,把下面这段 prompt 拼好传过去(`<...>` 占位符替换成主 agent 已收集到的实际值):
{{/claude}}
{{#codex}}
...
**项目规范基准**:第 1 / 5 条要对照仓库根 `{{AGENT_DOC}}` 与 `.github/PULL_REQUEST_TEMPLATE.md`——**直接 `Read` `{{AGENT_DOC}}` 的「设计实现规范」节与 `.github/PULL_REQUEST_TEMPLATE.md`(按标题定位,不要用记忆里的旧条款 / 旧编号,规范条数会增改),逐条核对**。其中**红线条必查**(按小标题语义定位,不靠编号):xdt-updater 改动声明、跨平台双端兼容、优先用代码保证确定性、throwIpcError、主题 token、maker-core 指标(缓存率/性能/准确性)、系统提示词改动须 Lizi 确认、DB migration、凭证不入仓。

0. 历史讨论的承接(最优先)
0.5. 代码与描述一致性 + PR 目的单一性(对账 + 搭便车拆分判定)
0.6. 重构他人历史功能的前置沟通门(git blame 定权属 + 6 项澄清,命中且无对齐证据即 `[阻断]`)
1. 是否符合项目规范(对照 `{{AGENT_DOC}}`「设计实现规范」与 `.github/PULL_REQUEST_TEMPLATE.md` 全部条款,红线条逐条核)
...
{{#claude}}
- **auto 批处理**(subagent 带 `isolation: "worktree"`,见「候选批处理」阶段 2)→ 填:`你运行在本仓库的一个独立 git worktree(当前目录即仓库根),代码尚未检出——第一步先跑 node scripts/review-pr/checkout.mjs <PR_NUMBER> --detach 把 PR 代码检出到当前 worktree(detached HEAD、不建分支),确认输出 ok:true 再开始审查。这一步是「只读不写」约束的唯一例外:它只切换你这个 worktree 检出的内容,不碰主仓库工作树、不修改任何文件内容。`
{{/claude}}
...
{{#claude}}
- **auto 批处理** → 填:`检出代码后,自己跑 node scripts/review-pr/context.mjs <PR_NUMBER>(不带 --scan),从输出读 meta.body(PR Description)、files(改动文件列表)、history(评论 / review thread 解决状态 / 提交时间线),自行整理历史脉络(从新到老)再开始审查;第 0 条「历史承接」以你自己拉的这份为准。`——主 agent **不代拉全文**,PR 的 description 与讨论全文只进这个 PR 自己的隔离上下文,这是防主 session 跨 PR 串扰的关键防线,不许图省事在主 session 里拉全量再贴过来。
{{/claude}}
...
1. **是否符合项目规范**:**`Read` 仓库根 `{{AGENT_DOC}}` 的「设计实现规范」节与 `.github/PULL_REQUEST_TEMPLATE.md`,以现行内容逐条核对**(不要用本 skill 或记忆里的旧编号——规范条数会增改)。尤其 render/main 解耦、统一 logger、跨平台、UI 不跳变、主题 token,以及红线条(xdt-updater / 跨平台 / 优先用代码保证确定性 / throwIpcError / 主题 token / maker-core 指标 / 系统提示词 / DB migration / 凭证不入仓)。maker package 改动看 package 自己的边界规则。
...
- **私聊提交者(先做,结果要进群消息)**:跑 `node scripts/review-pr/resolve-author-feishu.mjs <PR_NUMBER>`——匹配键按优先级:**① PR 作者 GitHub 账号**(名录行自带 `[@login]`,不受同事用个人邮箱提交影响,主键)、② commits 的 git 邮箱(辅助)。名录走本地 clone(`~/.xdmaker/org-rosters/`,每次自动 pull 最新、首次自动 SSH clone,gh api 兜底——本机需能以 SSH 读这两个仓库),命中行原文在 `matched[].line`(`matchedBy=github-login` 的行优先采信)。
...
{{#claude}}
> - **auto 批处理**:主树全程没 checkout、没建 `pr-<N>` 分支——合并过就 `cleanup.mjs --original <分支> --sync-main`(不加 `--pr`),一个都没合并就只调 `release-lock.mjs`。审查用的临时 worktree 由 `Agent` 工具自动回收,不用手动删;收尾时跑一眼 `git worktree list`,若发现明显是**上轮异常中断**残留的审查 worktree(路径在 `.claude/worktrees/` 下),用 `git worktree remove --force <path>` 清掉,认不准归属就别动、汇总里提一句让 owner 看。
{{/claude}}
>
