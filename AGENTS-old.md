...
| Command | Description |
|---------|-------------|
| `pnpm restart:desktop:remote` | **默认**；Agent 启动 / 重启桌面端连**远程 API**；补 `.env`、停所有 Cindy Electron dev 进程、在真 TTY 下启动，并等待窗口与 auth/localDb 启动完成（已登录时必须等数据库就绪；最多 120 秒，迁移失败、早退或超时都会在命令行报错）；支持 `--region=cn|global`（默认 `cn`） |
| `pnpm restart:desktop:local` | **仅用户明确要求本地时**；连**本地 server**（`http://localhost:3333`）；同样停旧进程、真 TTY 启动并等待窗口与 auth/localDb 启动完成；local 端点由 dev 脚本链自动生成的 `config/endpoint.local.json`(gitignored)承载；agent 只起客户端，本地 server 仍由用户自己起 |
...
背景：dev 和 release（正式安装版）默认共用同一个 userData / SQLite 数据库；多开时定时任务靠 DB 级原子认领互斥，但**旧 release 包没有认领逻辑**，过渡期需要下面的参数配合。
...
- restart 返回成功表示主窗口已经 `ready-to-show`，且 auth 已完成启动判断；若已有登录态，还必须等 localDb 初始化 / migration 成功。数据库迁移失败会把原始错误码与原因回传到当前命令行并非零退出；未登录时以可操作的登录页为成功终态。如果依赖、Forge 或 Electron 早退，或 120 秒内未完成上述启动，命令会失败并提示检查新终端和 `apps/desktop/logs/`。
...
4. **结束前必须 commit，正常发布走任务分支 + PR**：会话被**删除或归档**时，脏 worktree 的改动会先存为内容快照（`refs/xdt/snapshots/<sessionId>`）再删除目录（确认弹窗有警告；归档会话重开后可一键「恢复工作区」）。`/clear`、鉴权重连、app 退出等瞬态 close 不触发回收（2026-07 P0 重构）；在 worktree 里手动干活时可放 `.worktree-keep` 哨兵文件豁免一切自动回收。**PR merged / closed 不等于 Cindy 会话已结束**：只要 owning session 仍是 active，任何外部 Git cleanup（包括通用 cleanup planner）都必须跳过该 `.cindy-worktrees` / `.xdt-worktrees` 目录及本地 `xdt/*` 分支，交给用户显式归档 / 删除会话时由产品回收；禁止手动 `git worktree remove` 造成 active session 的 cwd 悬空。仍然不要把未提交的成果留在 worktree 里收工；但不得为了回收 worktree 绕过默认 PR 流程。维护者明确选择直推 `main` 时，必须改走「提 PR」节的例外门禁。
...
对外版本发布、签名、渠道与商业分发属于维护者内部流程，本归档不保留操作细节。
